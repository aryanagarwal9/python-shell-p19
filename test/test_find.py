...
class TestFind(unittest.TestCase):
    ...
    def test_find_flag_not_provided(self):
        app = Find()
        self.assertRaises(FlagError, app.exec, args=['resources1', '*.txt'], stdin=None, out=self.out)
    ...
    def test_find_flag_not_on_correct_index(self, name, args, result):
        app = Find()
        self.assertRaises(result, app.exec, args=args, stdin=None, out=self.out)

    @parameterized.expand([
        ['less than 2 arguments', ['-name'], ValueError],
        ['more than 3 arguments', ['resources1', 'resources2', '-name', '*.txt'], ValueError]
    ])
